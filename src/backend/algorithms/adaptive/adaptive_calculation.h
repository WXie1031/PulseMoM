...
// These types are only used as pointers in the interface, so forward declarations are sufficient
typedef struct mom_solver mom_solver_t;
typedef struct peec_solver peec_solver_t;
// Note: fem_solver and fdtd_solver are not currently used in this header
