...
// 本地配置文件解析结构（不冲突于头文件中的 peec_config_t）
typedef struct {
...
int main(int argc, char* argv[]) {
    ...
    // 创建 PEEC solver 配置
    printf("[1/5] Creating PEEC solver configuration...\n");
    peec_config_t solver_config = {0};
    solver_config.formulation = PEEC_FORMULATION_CLASSICAL;  // 默认使用经典 PEEC
    solver_config.frequency = file_config.frequency;
    solver_config.circuit_tolerance = file_config.tolerance;
    solver_config.circuit_max_iterations = file_config.max_iterations;
    solver_config.mesh_density = file_config.mesh_density;
    solver_config.extract_resistance = true;
    solver_config.extract_inductance = true;
    solver_config.extract_capacitance = true;
    solver_config.extract_mutual_inductance = true;
    solver_config.extract_mutual_capacitance = true;
    solver_config.use_parallel = true;
    
    // 创建 PEEC solver
    printf("[2/5] Creating PEEC solver...\n");
    peec_solver_t* solver = peec_solver_create(&solver_config);
    if (!solver) {
    ...
    if (peec_solver_import_cad(solver, file_config.geometry_file, file_config.geometry_format) != STATUS_SUCCESS) {
    ...
    // 提取部分元件
    printf("[4/5] Extracting partial elements...\n");
    if (peec_solver_extract_partial_elements(solver) != STATUS_SUCCESS) {
        fprintf(stderr, "Error: Failed to extract partial elements\n");
        peec_solver_destroy(solver);
        return 1;
    }
    
    // 构建电路网络
    printf("[5/6] Building circuit network...\n");
    if (peec_solver_build_circuit_network(solver) != STATUS_SUCCESS) {
        fprintf(stderr, "Error: Failed to build circuit network\n");
        peec_solver_destroy(solver);
        return 1;
    }
    
    // 求解电路
    printf("[6/6] Solving circuit...\n");
    if (peec_solver_solve_circuit(solver) != STATUS_SUCCESS) {
        fprintf(stderr, "Error: Circuit solution failed\n");
        peec_solver_destroy(solver);
        return 1;
    }
    
    // 获取结果
    printf("[7/7] Retrieving results...\n");
    const peec_result_t* results = peec_solver_get_results(solver);
    if (!results) {
