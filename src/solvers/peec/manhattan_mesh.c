...
#define mesh_create(TYPE,DIM) mesh_create("manhattan_mesh", (TYPE))
...
static int add_vias_to_mesh(mesh_t* mesh, via_node_t* vias, int num_vias);
...
mesh_t* manhattan_mesh_generate(surface_t* surface, mesh_params_t* params) {
    if (!surface || !params) return NULL;
    
    mesh_t* mesh = mesh_create(MESH_TYPE_RECTANGULAR, 3);
    if (!mesh) return NULL;
    
    // Convert generic parameters to Manhattan-specific parameters
    manhattan_params_t manhattan_params = {0};
    manhattan_params.grid_resolution[0] = params->target_edge_length > 0 ? 
                                        params->target_edge_length : DEFAULT_GRID_RESOLUTION;
    manhattan_params.grid_resolution[1] = manhattan_params.grid_resolution[0];
    manhattan_params.grid_resolution[2] = params->layer_thickness > 0 ? 
                                         params->layer_thickness : DEFAULT_GRID_RESOLUTION;
    manhattan_params.adaptive_refinement = params->adaptive_refinement;
    manhattan_params.via_modeling = params->via_modeling;
    manhattan_params.frequency = params->frequency;
    manhattan_params.min_feature_size = params->min_feature_size;
    manhattan_params.refinement_levels = params->refinement_levels;
    ...
    if (manhattan_params.adaptive_refinement) {
        mesh = generate_adaptive_grid(surface, &manhattan_params);
    } else {
        mesh = generate_structured_grid(bbox_min, bbox_max, &manhattan_params);
    }
    
    if (!mesh) return NULL;
    
    // Add vias if enabled
    if (manhattan_params.via_modeling && params->vias) {
        add_vias_to_mesh(mesh, params->vias, params->num_vias);
    }
    
    // Generate layer stackup for multi-layer PCBs
    if (params->num_layers > 1) {
        generate_layer_stackup(mesh, params->layers, params->num_layers);
    }
    
    // Post-processing
    if (params->optimize_quality) {
        optimize_manhattan_quality(mesh, manhattan_params.max_aspect_ratio);
    }
...
static int add_vias_to_mesh(mesh_t* mesh, via_node_t* vias, int num_vias) {
    if (!mesh || !vias || num_vias <= 0) return -1;
    ...
    for (int i = 0; i < num_vias; i++) {
        ...
        for (int j = 0; j < mesh->num_elements; j++) {
            ...
            if (via->center[0] >= rect->center[0] - rect->dimensions[0]/2 &&
                ...
                int via_layer = (int)(via->center[2] / rect->dimensions[2]);
                if (via_layer == rect->layer_id) {
                    rect->via_flag = 1;
                    
                    // Add via vertices
                    int top_vertex = mesh_add_vertex(mesh, via->center[0], via->center[1], 
                                                    via->center[2] + via->height/2);
                    int bottom_vertex = mesh_add_vertex(mesh, via->center[0], via->center[1], 
                                                       via->center[2] - via->height/2);
                ...
                for (int neighbor = 0; neighbor < 4; neighbor++) {
                    if (rect->neighbors[neighbor] >= 0) {
                        ...
                        rectangle_node_t* neighbor_rect = 
                            &((rectangle_node_t*)mesh->element_data)[neighbor_idx];
...
static int generate_manhattan_from_geometry(geom_rectangle_t* rectangles, 
    ...
    for (int i = 0; i < num_rectangles; i++) {
        ...
        snap_to_manhattan_grid(rectangles[i].corner.coords, params->grid_resolution);
...
static int merge_rectangles(mesh_t* mesh, int rect1_idx, int rect2_idx) {
    ...
    mesh_remove_element(mesh, rect2_idx);
