...
// Coupling state
typedef struct {
    double complex** coupling_matrix_mom;   // MTL-MoM coupling matrix
    double complex** coupling_matrix_peec; // MTL-PEEC coupling matrix
    double complex* boundary_conditions;     // Shared boundary conditions
...
static int mtl_coupling_get_mom_port_info(mom_solver_t* mom_solver, int* num_ports, int** port_nodes, double** port_impedances) {
    ...
    *num_ports = mom_results->num_ports;
    log_info("MoM solver has %d ports", *num_ports);
...
static int mtl_coupling_get_peec_network_info(peec_solver_t* peec_solver, int* num_nodes, int* num_branches, 
    ...
    // Get circuit network from PEEC solver
    peec_circuit_network_t* network = peec_solver_get_circuit_network(peec_solver);
    if (!network) {
    ...
    *num_branches = network->num_elements;
    ...
    if (*num_nodes > 0 && node_ids) {
        *node_ids = (int*)malloc(*num_nodes * sizeof(int));
        ...
        memcpy(*node_ids, network->node_ids, *num_nodes * sizeof(int));
    }
    ...
    if (*num_branches > 0 && branch_nodes) {
        *branch_nodes = (int*)malloc(2 * *num_branches * sizeof(int)); // from, to pairs
        ...
        for (int i = 0; i < *num_branches; i++) {
            (*branch_nodes)[2*i] = network->branch_from_nodes[i];
            (*branch_nodes)[2*i + 1] = network->branch_to_nodes[i];
        }
    }
...
static int mtl_coupling_compute_field_interaction(mtl_solver_t* mtl_solver, mom_solver_t* mom_solver, mtl_coupling_state_t* state) {
    ...
    if (!state->coupling_matrix_mom && num_conductors > 0 && num_frequencies > 0) {
        state->coupling_matrix_mom = (double complex**)calloc(num_conductors, sizeof(double complex*));
        for (int i = 0; i < num_conductors; i++) {
            state->coupling_matrix_mom[i] = (double complex*)calloc(num_frequencies, sizeof(double complex));
        }
    }
    ...
    for (int i = 0; i < num_conductors; i++) {
        for (int freq_idx = 0; freq_idx < num_frequencies; freq_idx++) {
            ...
            double complex mtl_current = mtl_results->currents[freq_idx * num_conductors + i];
            double complex mtl_voltage = mtl_results->voltages[freq_idx * num_conductors + i];
            
            // Calculate induced fields from MoM results
            double complex induced_field = 0.0 + 0.0 * I;
            
            // Couple with MoM currents if available
            if (mom_results->currents && mom_results->num_currents > 0) {
                // Use mutual impedance concept for coupling
                for (int mom_idx = 0; mom_idx < mom_results->num_currents; mom_idx++) {
                    double complex mom_current = mom_results->currents[mom_idx];
                    // Simple coupling coefficient (would be spatial in real implementation)
                    double coupling_coeff = 0.1 * exp(-0.1 * abs(mom_idx - i));
                    induced_field += coupling_coeff * mom_current;
                }
            }
            
            // Store coupling coefficient
            if (state->coupling_matrix_mom && state->coupling_matrix_mom[i]) {
                state->coupling_matrix_mom[i][freq_idx] = induced_field / (mtl_current + 1e-12);
            }
            
            log_debug("MTL-MoM coupling: conductor %d, freq %.2e Hz, current %.3e A, induced field %.3e V/m",
                     i, frequency, cabs(mtl_current), cabs(induced_field));
        }
    }
...
static int mtl_coupling_compute_circuit_interaction(mtl_solver_t* mtl_solver, peec_solver_t* peec_solver, mtl_coupling_state_t* state) {
    ...
    if (!state->coupling_matrix_peec && num_conductors > 0 && num_frequencies > 0) {
        state->coupling_matrix_peec = (double complex**)calloc(num_conductors, sizeof(double complex*));
        for (int i = 0; i < num_conductors; i++) {
            state->coupling_matrix_peec[i] = (double complex*)calloc(num_frequencies, sizeof(double complex));
        }
    }
    ...
    for (int i = 0; i < num_conductors; i++) {
        for (int freq_idx = 0; freq_idx < num_frequencies; freq_idx++) {
            double complex voltage = mtl_results->voltages[freq_idx * num_conductors + i];
            double complex current = mtl_results->currents[freq_idx * num_conductors + i];
            double frequency = mtl_results->frequencies[freq_idx];
            
            // Calculate equivalent circuit parameters
            double complex impedance = voltage / (current + 1e-12); // Avoid division by zero
            double complex admittance = 1.0 / impedance;
            
            // Find corresponding PEEC node (simplified mapping)
            int peec_node = i % network->num_nodes; // Simple modulo mapping
            if (peec_node < network->num_nodes && network->node_voltages) {
                // Update PEEC node voltage based on MTL results
                double complex coupling_factor = 0.8; // Coupling strength
                network->node_voltages[peec_node] = coupling_factor * voltage;
            }
            ...
            log_debug("MTL-PEEC coupling: conductor %d -> node %d, freq %.2e Hz, Z=%.3e ohm",
                     i, peec_node, frequency, cabs(impedance));
        }
    }
...
static double mtl_coupling_compute_convergence(mtl_coupling_state_t* current_state, mtl_coupling_state_t* prev_state) {
    ...
    if (current_state->coupling_matrix_mom && prev_state->coupling_matrix_mom && 
        ...
        for (int i = 0; i < current_state->num_boundary_points; i++) {
            for (int j = 0; j < current_state->num_boundary_points; j++) {
                double complex current_val = current_state->coupling_matrix_mom[i][j];
                double complex prev_val = prev_state->coupling_matrix_mom[i][j];
                
                double diff = cabs(current_val - prev_val);
                double mag = cabs(prev_val) + 1e-12; // Avoid division by zero
    ...
    if (current_state->coupling_matrix_peec && prev_state->coupling_matrix_peec && 
        ...
        for (int i = 0; i < current_state->num_boundary_points; i++) {
            for (int j = 0; j < current_state->num_boundary_points; j++) {
                double complex current_val = current_state->coupling_matrix_peec[i][j];
                double complex prev_val = prev_state->coupling_matrix_peec[i][j];
    ...
    if (current_state->boundary_conditions && prev_state->boundary_conditions &&
        ...
        for (int i = 0; i < current_state->num_boundary_points; i++) {
            ...
            double mag = cabs(prev_state->boundary_conditions[i]) + 1e-12;
