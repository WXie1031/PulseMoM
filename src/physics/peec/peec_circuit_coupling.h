...
#include "../../solvers/peec/peec_advanced.h"  // For spice_format_t definition
...
typedef struct {
    ...
    CDOUBLE *voltage_harmonics;           // Voltage harmonics array
    CDOUBLE *current_harmonics;           // Current harmonics array
    CDOUBLE *charge_harmonics;            // Charge harmonics array
    int num_harmonics;                    // Number of harmonics
    
    // Jacobian matrices for Newton-Raphson
    CDOUBLE *conductance_matrix;          // dI/dV matrix
    CDOUBLE *capacitance_matrix;         // dQ/dV matrix
...
typedef struct {
    ...
    CDOUBLE *impedance_frequency;        // Frequency-dependent impedance
    CDOUBLE *admittance_frequency;        // Frequency-dependent admittance
...
typedef struct {
    ...
    CDOUBLE *impedance_matrix;           // Z-matrix (num_ports × num_ports)
    CDOUBLE *admittance_matrix;         // Y-matrix (num_ports × num_ports)
    CDOUBLE *scattering_matrix;          // S-matrix (num_ports × num_ports)
    CDOUBLE *abcd_matrix;                // ABCD-matrix (2×2 for 2-port)
    ...
    CDOUBLE **time_domain_matrix;        // Time-domain matrices
...
typedef struct {
    ...
    CDOUBLE input_impedance;              // Input impedance (Ω)
    CDOUBLE output_impedance;             // Output impedance (Ω)
    CDOUBLE voltage_gain;                 // Voltage gain
    CDOUBLE current_gain;                 // Current gain
    CDOUBLE power_gain;                   // Power gain
    CDOUBLE s11;                          // Input reflection
    CDOUBLE s21;                          // Forward transmission
    CDOUBLE s12;                          // Reverse transmission
    CDOUBLE s22;                          // Output reflection
...
typedef struct {
    ...
    CDOUBLE optimum_source_impedance;    // Optimum source impedance
