/********************************************************************************
 * Python ctypes interface for Satellite MoM/PEEC C Library
 *
...
class SatelliteMoMPEECInterface:
    """
    Python interface to Satellite MoM/PEEC C library using ctypes.
    ...
    def _load_library(self, library_path: Optional[str] = None):
        ...
        if library_path is None or not os.path.exists(library_path):
            raise FileNotFoundError(f"Could not find satellite MoM/PEEC library at {library_path}")
    ...
    def _get_default_library_paths(self) -> list:
        ...
        if platform.system() == "Windows":
            lib_name = "satellite_mom_peec.dll"
            possible_paths = [
            ...
            lib_name = "libsatellite_mom_peec.dylib"
            possible_paths = [
            ...
            lib_name = "libsatellite_mom_peec.so"
            possible_paths = [
                os.path.join(base_dir, "build", lib_name),
                os.path.join(base_dir, "lib", lib_name),
                os.path.join("/usr/local/lib", lib_name),
    ...
    def _setup_function_signatures(self):
        ...
        self.lib.satellite_get_error_string.restype = ctypes.c_char_p
        self.lib.satellite_get_error_string.argtypes = [ctypes.c_int]
        
        self.lib.satellite_get_version.restype = ctypes.c_char_p
        self.lib.satellite_get_version.argtypes = []
        
        # Geometry functions
        self.lib.satellite_load_stl.restype = ctypes.c_int
        self.lib.satellite_load_stl.argtypes = [ctypes.c_void_p, ctypes.c_char_p]
        
        self.lib.satellite_set_material.restype = ctypes.c_int
        self.lib.satellite_set_material.argtypes = [ctypes.c_void_p, ctypes.c_void_p]
        
        self.lib.satellite_generate_mesh.restype = ctypes.c_int
        self.lib.satellite_generate_mesh.argtypes = [ctypes.c_void_p, ctypes.c_void_p]
        
        self.lib.satellite_get_mesh_info.restype = ctypes.c_int
        self.lib.satellite_get_mesh_info.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int)]
        
        # Solver configuration
        self.lib.satellite_configure_solver.restype = ctypes.c_int
        self.lib.satellite_configure_solver.argtypes = [ctypes.c_void_p, ctypes.c_void_p]
        
        self.lib.satellite_set_excitation.restype = ctypes.c_int
        self.lib.satellite_set_excitation.argtypes = [ctypes.c_void_p, ctypes.c_void_p]
        
        # Simulation execution
        self.lib.satellite_run_simulation.restype = ctypes.c_int
        self.lib.satellite_run_simulation.argtypes = [ctypes.c_void_p]
        
        # Results retrieval
        self.lib.satellite_get_currents.restype = ctypes.c_int
        self.lib.satellite_get_currents.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_void_p)]
        
        self.lib.satellite_get_fields.restype = ctypes.c_int
        self.lib.satellite_get_fields.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.POINTER(ctypes.c_void_p)]
        
        self.lib.satellite_get_performance.restype = ctypes.c_int
        self.lib.satellite_get_performance.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_void_p)]
        
        # Memory management
        self.lib.satellite_free_currents.restype = None
        self.lib.satellite_free_currents.argtypes = [ctypes.c_void_p]
        
        self.lib.satellite_free_fields.restype = None
        self.lib.satellite_free_fields.argtypes = [ctypes.c_void_p]
        
        self.lib.satellite_free_performance.restype = None
        self.lib.satellite_free_performance.argtypes = [ctypes.c_void_p]
    
    def create_simulation(self) -> int:
        """Create a new simulation instance. Returns handle to simulation."""
        return self.lib.satellite_simulation_create()
    
    def destroy_simulation(self, sim_handle: int):
        """Destroy a simulation instance."""
        self.lib.satellite_simulation_destroy(sim_handle)
    
    def get_error_string(self, error_code: int) -> str:
        """Get human-readable error string."""
        return self.lib.satellite_get_error_string(error_code).decode('utf-8')
    
    def get_version(self) -> str:
        """Get library version string."""
        return self.lib.satellite_get_version().decode('utf-8')
    
    def load_stl(self, sim_handle: int, stl_filename: str) -> int:
        """Load STL geometry file."""
        return self.lib.satellite_load_stl(sim_handle, stl_filename.encode('utf-8'))
    ...
    def set_material(self, sim_handle: int, material: Dict[str, Any]) -> int:
        ...
        return self.lib.satellite_set_material(sim_handle, ctypes.byref(mat))
    ...
    def generate_mesh(self, sim_handle: int, mesh_params: Dict[str, Any]) -> int:
        ...
        return self.lib.satellite_generate_mesh(sim_handle, ctypes.byref(params))
    ...
    def get_mesh_info(self, sim_handle: int) -> Tuple[int, int]:
        ...
        error = self.lib.satellite_get_mesh_info(sim_handle, ctypes.byref(num_vertices), ctypes.byref(num_triangles))
        if error != 0:
    ...
    def configure_solver(self, sim_handle: int, solver_config: Dict[str, Any]) -> int:
        ...
        return self.lib.satellite_configure_solver(sim_handle, ctypes.byref(config))
    ...
    def set_excitation(self, sim_handle: int, excitation: Dict[str, Any]) -> int:
        ...
        return self.lib.satellite_set_excitation(sim_handle, ctypes.byref(exc))
    
    def run_simulation(self, sim_handle: int) -> int:
        """Run the electromagnetic simulation."""
        return self.lib.satellite_run_simulation(sim_handle)
    
    def get_currents(self, sim_handle: int) -> Dict[str, np.ndarray]:
        """Get surface current distribution."""
        currents_ptr = ctypes.c_void_p()
        error = self.lib.satellite_get_currents(sim_handle, ctypes.byref(currents_ptr))
        ...
        # Free the C memory
        self.lib.satellite_free_currents(currents_ptr)
    ...
    def get_fields(self, sim_handle: int, observation_points: np.ndarray) -> Dict[str, np.ndarray]:
        ...
        error = self.lib.satellite_get_fields(sim_handle, ctypes.byref(points), ctypes.byref(fields_ptr))
        ...
        # Free the C memory
        self.lib.satellite_free_fields(fields_ptr)
    ...
    def get_performance(self, sim_handle: int) -> Dict[str, Any]:
        ...
        error = self.lib.satellite_get_performance(sim_handle, ctypes.byref(perf_ptr))
        ...
        # Free the C memory
        self.lib.satellite_free_performance(perf_ptr)
        
        return result

# Convenience function for running complete satellite simulation
def run_satellite_c_simulation(stl_file: str, frequency: float = 10e9, 
                             solver_type: str = 'mom', **kwargs) -> Dict[str, Any]:
    """
    Run a complete satellite simulation using the C library.
...
if __name__ == "__main__":
    print("Testing Satellite MoM/PEEC C Library Interface...")
