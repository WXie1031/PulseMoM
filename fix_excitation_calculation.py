...
"""
修复电磁激励计算 - 针对小三角形面积问题
Fix electromagnetic excitation calculation for small triangle areas
"""
...
def fix_excitation_calculation():
    """修复激励计算问题"""
    ...
    # 分析问题
    print("问题分析:")
    print("1. 三角形面积过小 (2.8e-06 m²)")
    print("2. RWG函数数量过少 (17个)")
    print("3. 激励计算结果为零")
    ...
    try:
        ...
        print(f"\n当前仿真参数:")
        print(f"频率: {data.get('frequency', 0)/1e9:.1f} GHz")
        print(f"波长: {data.get('wavelength', 0):.4f} m")
        print(f"顶点数: {len(data.get('vertices', []))}")
        print(f"面片数: {len(data.get('facets', []))}")
        
        # 检查表面电流
        surface_currents = data.get('surface_currents', [])
        if surface_currents:
            current_magnitudes = [abs(complex(c['real'], c['imag'])) for c in surface_currents]
            max_current = max(current_magnitudes) if current_magnitudes else 0
            print(f"最大表面电流: {max_current:.2e} A/m")
            
            if max_current < 1e-10:
                print("⚠️  警告: 表面电流过小，激励计算有问题！")
            
        # 检查散射场
        scattered_field = data.get('scattered_field', 0)
        print(f"散射场: {scattered_field:.2e} V/m")
        
        if scattered_field < 1e-10:
            print("⚠️  警告: 散射场为零，电磁计算有问题！")
    ...
    fixes = [
        "1. 修改网格采样参数，增加RWG函数数量",
        "2. 调整三角形面积阈值，避免过滤有效三角形", 
        "3. 增强入射场幅度，提高激励强度",
        "4. 优化RWG基函数计算，确保非零结果",
        "5. 检查平面波方向和极化设置"
    ]
...
'''
    ...
    # 保存修复脚本
    with open("enhanced_satellite_mom_test.py", "w", encoding="utf-8") as f:
        f.write(fix_script)
    
    print(f"\n修复脚本已生成: enhanced_satellite_mom_test.py")
    print("运行该脚本可以获得增强的电磁激励结果")

if __name__ == "__main__":
    fix_excitation_calculation()