...
class MTL():
    ...
    def add_external_field(self, field : Field, distances: npt.NDArray[np.float64], field_localization: list[npt.NDArray[np.float64]] = []):
        # Support both symbolic (sympy) fields and numeric callable fields from external solvers
        ...
        def _is_sympy_expr(obj):
            return isinstance(obj, sp.Expr) or isinstance(obj, sp.Symbol)

        is_symbolic = _is_sympy_expr(getattr(field, 'e_x', None)) and \
                       _is_sympy_expr(getattr(field, 'e_y', None)) and \
                       _is_sympy_expr(getattr(field, 'e_z', None))
        ...
        if is_symbolic:
            for n in range(self.number_of_conductors):
                ...
                l = np.abs(np.array([xi,yi,zi]))
                l /= np.linalg.norm(l)
                # Use last available x0,y0,z0 from loop above
            ...
            # Numeric path: field.e_x, e_y, e_z are callables f(x, y, z, t, v?) -> float.
            # We ignore symbolic substitution and compute numeric differences and line integrals.
            # Expected signature: f(x, y, z, t) or f(x, y, z, t, v). We handle both.
            def _call_field(f, x0, y0, z0, t0, v0):
                try:
                    return f(x0, y0, z0, t0, v0)
                except TypeError:
                    return f(x0, y0, z0, t0)

            for n in range(self.number_of_conductors):
                for nz in indices:
                    x0, y0, z0 = self.u[nz, :]
                    xi, yi, zi = distances[n, nz, :]
                    du_half = 0.5 * self.du[nz, :]
                    vloc = float(np.max(vu[nz]))

                    # Longitudinal component as difference at segment ends, projected onto u
                    unit_du = self.du[nz, :] / np.linalg.norm(self.du[nz, :])

                    def e_vec_at(xp, yp, zp, tp):
                        ex = _call_field(field.e_x, xp, yp, zp, tp, vloc)
                        ey = _call_field(field.e_y, xp, yp, zp, tp, vloc)
                        ez = _call_field(field.e_z, xp, yp, zp, tp, vloc)
                        return np.array([ex, ey, ez], dtype=float)

                    def eL_t(tp):
                        e_plus = e_vec_at(x0 + xi - du_half[0], y0 + yi - du_half[1], z0 + zi - du_half[2], tp)
                        e_minus = e_vec_at(x0 - du_half[0], y0 - du_half[1], z0 - du_half[2], tp)
                        return float((e_plus - e_minus).dot(unit_du))

                    self.e_L[n, nz] = eL_t

                    # Transverse component as line integral along displacement l using simple 3-point Simpson rule
                    lvec = np.array([xi, yi, zi], dtype=float)
                    if np.linalg.norm(lvec) == 0.0:
                        def eT_t_zero(tp):
                            return 0.0
                        self.e_T[n, nz] = eT_t_zero
                    else:
                        lunit = lvec / np.linalg.norm(lvec)

                        def eT_t(tp):
                            # Parametrize r(s) = r0 + s*lvec, s in [0,1]
                            r0 = np.array([x0, y0, z0], dtype=float)
                            # 3-point Simpson's rule
                            s0, s1, s2 = 0.0, 0.5, 1.0
                            w0, w1, w2 = 1.0, 4.0, 1.0
                            pts = [r0 + s0*lvec, r0 + s1*lvec, r0 + s2*lvec]
                            ev = []
                            for p in pts:
                                ev.append(e_vec_at(p[0], p[1], p[2], tp))
                            integrand = [evk.dot(lunit) for evk in ev]
                            return float(np.linalg.norm(lvec) * (w0*integrand[0] + w1*integrand[1] + w2*integrand[2]) / 6.0)

                        self.e_T[n, nz] = eT_t

                # Last point for transverse term
                x0, y0, z0 = self.u[-1, :]
                xi, yi, zi = distances[n, -1, :]
                lvec = np.array([xi, yi, zi], dtype=float)
                if np.linalg.norm(lvec) == 0.0:
                    def eT_t_last(tp):
                        return 0.0
                    self.e_T[n, -1] = eT_t_last
                else:
                    lunit = lvec / np.linalg.norm(lvec)
                    vloc = float(np.max(vu[-1]))
                    ...
                    def eT_t_last(tp):
                        ...
                        ev = []
                        for p in pts:
                            ex = _call_field(field.e_x, p[0], p[1], p[2], tp, vloc)
                            ey = _call_field(field.e_y, p[0], p[1], p[2], tp, vloc)
                            ez = _call_field(field.e_z, p[0], p[1], p[2], tp, vloc)
                            ev.append(np.array([ex, ey, ez], dtype=float))
                        integrand = [evk.dot(lunit) for evk in ev]
                        return float(np.linalg.norm(lvec) * (w0*integrand[0] + w1*integrand[1] + w2*integrand[2]) / 6.0)
