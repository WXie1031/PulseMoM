function spice_subckt_vi(Rmtx,Lmtx,Cmtx, Rfit,Lfit, bran_name,nod_name, ...
    nod_start, nod_end, bran_out, nod_out, Uind,tui, fpath, fname,Rgrid,Lgrid,Cgrid, bran_name_grid,nod_name_grid, ...
...
for ik = 1:Nc1
    ...
    Lm(ik,1:Nc2) = cal_L_fila(pt_start(ik,1:3), pt_end(ik,1:3), dv(ik,1:3), len(ik), re(ik), ...
...
spice_subckt_self_vf_vi(Rmtx,Lmtx,Cmtx, Rfit,Lfit, bran_name,nod_name, ...
    nod_start, nod_end,nod_gnd_grid,Nnod_gnd_grid, bran_out, nod_out, Uind,tui, fpath, fname,Rgrod,Rsoil,Nbran1,Nbran2,re,len);
