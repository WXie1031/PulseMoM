function [R0fit,L0fit, Rvfit,Lvfit, Zvfit] = vectfit_main_Zin_backup(Rself, Lself, ...
    frq, Nfit)
...
for ik = 1:Nc
    ...
    if Rself(ik,2)~=0
        ...
        if (Rtmp-Rself(ik,1))/Rself(ik,1)>0.4  %  >40%
            ...
            Zs = zeros(1,1,Nf);
            Zs(1,1,1:Nf) = Rself(ik,1:Nf) + 1j*2*pi*frq.*(Lself(ik,1:Nf)-Lself(ik,1));
            
            [R0, ~, Rn, Ln, Ztmp] = vecfit_kernel_Zin(Zs, frq, Nfit, VFmode);
            R0fit(ik) = R0-sum(Rn);
            L0fit(ik) = Lself(ik,1);
            Rvfit(ik,1:Nfit) = Rn;
            Lvfit(ik,1:Nfit) = Ln;
            Zvfit(ik,:) = squeeze(Ztmp(1,1,:))+2*pi*frq'*L0fit(ik);
        elseif (Rtmp-Rself(ik,1))/Rself(ik,1)<0.4 && ...
                (Rtmp-Rself(ik,1))/Rself(ik,1)>0.1 %  >40%
            
            Zs = zeros(1,1,Nf);
            Zs(1,1,1:Nf) = Rself(ik,1:Nf) + 1j*2*pi*frq.*(Lself(ik,1:Nf)-Lself(ik,1));
            
            [R0, ~, Rn, Ln,Ztmp] = vecfit_kernel_Zin(Zs, frq, 1, VFmode);
            R0fit(ik) = R0-sum(Rn);
            L0fit(ik) = Lself(ik,1);
            Rvfit(ik,1) = Rn;
            Lvfit(ik,1) = Ln;
            Zvfit(ik,:) = squeeze(Ztmp(1,1,:))+2*pi*frq'*L0fit(ik);
        elseif (Rtmp-Rself(ik,1))/Rself(ik,1)<0.1
            R0fit(ik) = Rself(ik,1);
            Zvfit(ik,:) =  Rself(ik,1) + 1j*2*pi*frq.*Lself(ik,1);
        end
