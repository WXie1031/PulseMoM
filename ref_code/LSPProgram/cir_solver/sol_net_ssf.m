...
% So      Source
function [Cssf,Gssf,Bssf, Amtx, id_sv,id_si, nod_new] = sol_net_ssf(Lmtx,Pmtx, ...
    Rec,Lec,Cec, nod_start,nod_end, nod_P, nod_gnd,nod_sr, nod_ec, sr_type)
...
%% generate state space variables
Cssf = [-Amtx'  -blkdiag(Rpeec, diag(Rec)); 
    zeros(Nn,Nn)  -Amtx];

Gssf = [zeros(Nb,Nn)  -Lmtx;
    blkdiag(Cmtx, diag(Cec))  zeros(Nn,Nb)];
