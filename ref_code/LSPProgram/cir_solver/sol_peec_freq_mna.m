...
function [Ibf, Vnf] = sol_peec_freq_mna( Rpeec,Lpeec,Ppeec, TdL,TdP, Rec,Lec,Cec, ...
...
for ik = 1:Nfh
    ...
    S = Ss.*exp(-1j*w(ik)*TdP);
    
    Zall = blkdiag(Z, diag(Rec), 1j*w(ik)*diag(Lec));
    Call = blkdiag(1j*w(ik)*Cs, 1j*w(ik)*diag(Cec));
    Sall = blkdiag(S, eye(length(Cec)));
    
    %out = [ -A' -Z ; 1j*w*F  -S*A; ]\[ Vs; S*Is ] ;
    
    out = [ -Amtx' -Zall ; Call  -S*Amtx; ]\[ Vs; Sall*Is ] ;
