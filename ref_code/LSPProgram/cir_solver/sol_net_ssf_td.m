...
% So      Source
function [Cssf,Gssf,Bssf, Amtx, id_sv,id_si, TdPr,TdGPr, nod_new] = sol_net_ssf_td(Lmtx,Pmtx,Pg, ...
...
A0 = inv([zeros(Nn,Nb) -Lnr])*[-Amtx'  -blkdiag(Rpeec, diag(Rec)); 
    zeros(Nn,Nn)  -Amtx];
...
Gssf = [zeros(Nb,Nn)  -Lmtx;
    blkdiag(Cpeec, diag(Cec))  zeros(Nn,Nb)];
