function [Rmesh, Lmesh, Rself, Lself] = main_mesh2d_part(type_flag, pt2D, ...
    dim1, dim2, Rin_pul, Lin_pul, sig, mur, len, freq)
%  Function:       mesh2d_main_partial
...
indrec = type_flag == 1002;
indagi = type_flag == 1003;
...
for ig = 1:Nf
    ...
    %f0 = 500;
    [RmeshMF(:,:,ig), LmeshMF(:,:,ig)] = cal_RL_mesh2d_partial(...
        p2Dcir, rocir, ricir, Rpmcir, murcir, ...
        p2Drec, wrec, hrec, Rpmrec, sigrec, murrec, ...
        p2Dagi, wagi, tagi, Rpmagi, sigagi, muragi, len, freq(ig));
...
elseif freq(Nf)<fi
    ...
    for ik = 1:Nc
        for ig = 1:Nc
            Rmesh(ik,ig) = interp1(freq, squeeze(RmeshMF(ik,ig,:)), fi, 'linear');
        end
    end
end
...
elseif freq(Nf)<fi
    ...
    for ik = 1:Nc
        for ig = 1:Nc
            Lmesh(ik,ig) = interp1(freq, squeeze(LmeshMF(ik,ig,:)), fi, 'linear');
        end
    end
end
