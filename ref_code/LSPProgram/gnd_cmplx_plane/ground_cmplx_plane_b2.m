function [Rg, Lg, Rgself, Lgself] = ground_cmplx_plane_b2(pt_start, pt_end, dv, re, len,...
    sig_g1,epr_g1,dg1, sig_g2,epr_g2, frq, offset)
...
rg_eq = rg1.* ( (rg1+rg2)-(rg1-rg2).*exp(-1j*2.*rg1.*dg1) ) ./ ...
    ( (rg1+rg2)+(rg1-rg2).*exp(-1j*2.*rg1.*dg1) );

cplx_depth = 1./rg_eq;   % complex skin depth
...
rg1_fm = sqrt( 1j.*wm.*mu0.*(sig_g1 + 1j.*wm.*ep0*epr_g1) );
rg2_fm = sqrt( 1j.*wm.*mu0.*(sig_g2 + 1j.*wm.*ep0*epr_g2) );
rg_eq_fm = rg1_fm.* ( (rg1_fm+rg2_fm)-(rg1_fm-rg2_fm).*exp(-1j*2.*rg1_fm.*dg1) ) ./ ...
    ( (rg1_fm+rg2_fm)+(rg1_fm-rg2_fm).*exp(-1j*2.*rg1_fm.*dg1) );
cplx_depth_fm = 1./rg_eq_fm;   % complex skin depth
