function [Rg,Lg, Rgself,Lgself] = grid_cmplx_plane_backup(pt_start,pt_end, dv, re, len,...
...
for ik = 1:Nc
    % calculate inductance using filament model
    Lg_cmplx(:,ik) = cal_L_fila( pt_start(ik,1:3), pt_end(ik,1:3), dv(ik,1:3), len(ik), re(ik), ...
        pt_start_img, pt_end_img, dv_img, len_img, re_img);
