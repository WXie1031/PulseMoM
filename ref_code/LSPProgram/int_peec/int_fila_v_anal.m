function int = int_fila_v_anal(ps1,ps2,ls, pf1,pf2,lf)
%  Function:       int_fila_v_anal
%  Description:    Calculate intergral between two perpendicular lines by
%                  modifying intergral formulas for arbitrary lines
%  Calls:
%  Input:          ps1  --  coordinate of start point of source line
%                  ps2  --  coordinate of end point of source line
%                  ls   --  length of source line
%                  pf1  --  coordinate of start point of field line
%                  pf2  --  coordinate of end point of field line
%                  lf   --  length of field line
...
OMG(id_dp) = atan(((u(id_dp)+ls).*(v(id_dp)+lf(id_dp)))./(d(id_dp).*R1(id_dp))) ...
    - atan(((u(id_dp)+ls).*v(id_dp))./(d(id_dp).*R2(id_dp))) ...
    + atan(((u(id_dp).*v(id_dp)))./(d(id_dp).*R3(id_dp))) ...
    - atan((u(id_dp).*(v(id_dp)+lf(id_dp)))./(d(id_dp).*R4(id_dp)));


int = ( 2*( (u+ls).*atanh(lf./(R1+R2+aa)) + (v+lf).*atanh(ls./(R1+R4+aa)) ...
    - u.*atanh(lf./(R3+R4+aa)) - v.*atanh(ls./(R2+R3+aa)) ) - OMG.*d) ;
