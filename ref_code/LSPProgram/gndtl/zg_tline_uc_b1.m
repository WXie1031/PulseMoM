function [Rmg,Lmg, Zg, Rsg,Lsg] = zg_tline_uc_b1(Rmg, Lmg, pt_2d,re, len, ...
    sig_soil, epr_soil, frq)
%  Function:       zg_tline_uc_b1
    ...
    if pt_2d(ig,2)<0
        ...
        for ik = 1:Nuc
            int_tmp(ik,:) = integral(@(lamda) ...
                (exp(H1(ik)*sqrt(lamda.^2+rg.^2)) .* cos(lamda.*dx(ik)) ...
                ./ (lamda+sqrt(lamda.^2+rg.^2))) ,0,Inf,'ArrayValued',true);
        end
        
        wm_tmp = ones(Nuc,1)*wm;
        Ztmp = 1j*mu0/(2*pi)*wm_tmp.* ( (besselk(0,d*rg)-besselk(0,D*rg)) ...
            + 2*int_tmp );
