function [RmeshMF,LmeshMF] = main_mesh2d_nn_self_sub(RmeshMF,LmeshMF, ...
    shp_id, pt_2d, dim1, dim2, Rpul, sig,mur, len, frq_cal, frq_nn, Nneuron, Lm)
...
for ik = 1:Nc
    ...
    if Nfcal < 20
        ...
        rat_Ltr = interp1(frq_cal, squeeze(rat_L(ik,ik,1:Nfcal))', frq_L, 'linear');
    else
        rat_Rtr = squeeze(rat_R(ik,ik,1:Nfcal))'-1;
        idL = find(frq_cal>=10e3);
        rat_Ltr = squeeze(rat_L(ik,ik,idL))';
    end
    
    %net_R = nn_simpe(frq_cal,squeeze(rat_R(ik,ik,1:Nfcal))'-1,Nnlayer);
     x_input = [1./sqrt(frq_R); (frq_R).^-(1/3)];
     
    net_R = nn_simpe(x_input, rat_Rtr, Nneuron);
    
    x_nn = [1./sqrt(frq_nn); (frq_nn).^-(1/3)];
    rat_R(ik,ik,Nfcal+1:Nf) = net_R(x_nn)+1;
    % check the error
%     for ih = Nfcal+1:Nf
%         rat_R(ik,ik,ih) = min(max(rat_R(ik,ik,ih),rat_R(ik,ik,ih-1)), 1.5*rat_R(ik,ik,ih-1));
%     end

    
    net_L = nn_simpe(1./sqrt(frq_L), rat_Ltr, Nneuron-1);
    rat_L(ik,ik,Nfcal+1:Nf) = net_L(1./sqrt(frq_nn));
    % check the error
%     for ih = Nfcal+1:Nf
%         if rat_L(ik,ik,ih)<=0
%             rat_L(ik,ik,ih) = rat_L(ik,ik,ih-1);
%         else
%             rat_L(ik,ik,ih) = max(min(rat_L(ik,ik,ih),rat_L(ik,ik,ih-1)),0.75*rat_L(ik,ik,ih-1));
%         end
%     end
