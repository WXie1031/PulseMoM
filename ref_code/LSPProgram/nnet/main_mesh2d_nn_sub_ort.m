function [RmeshMF,LmeshMF] = main_mesh2d_nn_sub_ort(RmeshMF,LmeshMF, ...
...
Rprox = zeros(Nc,Nc,Nf);
Lprox = zeros(Nc,Nc,Nf);
Rprox(:,:,1:Nfcal) = RmeshMF(:,:,1:Nfcal)-Rmtx(:,:,1:Nfcal);
Lprox(:,:,1:Nfcal) = LmeshMF(:,:,1:Nfcal)-Lmtx(:,:,1:Nfcal);
...
for ik = 1:Nc
    ...
    if Nfcal < 20
        Rprox_tmp = interp1(frq_cal, squeeze(Rprox(ik,ik,1:Nfcal))', frq_R, 'linear');
    else
        Rprox_tmp = squeeze(Rprox(ik,ik,1:Nfcal))';
    end
...
% find the Penetration frequency
    ...
    x_input = [1./sqrt(frq_R(id));];
%     x_input = [1./sqrt(frq_R(id)); (frq_R(id)).^-(1/3)];
    
    net_R = nn_simpe(x_input, Rprox_tmp(id), Nnlayer);
    
    x_nn = [1./(sqrt(frq_nn));];
    Rprox(ik,ik,Nfcal+1:Nf) = net_R(x_nn);
    % check the error
%     for ih = Nfcal+1:Nf
%         dif_R(ik,ik,ih) = min(max(dif_R(ik,ik,ih),dif_R(ik,ik,ih-1)), 5*dif_R(ik,ik,ih-1));
%     end
    ...
    for ig = ik:Nc
        ...
        if Nfcal < 20
            Lprox_tmp = interp1(frq_cal, squeeze(Lprox(ik,ig,1:Nfcal))', frq_L, 'linear');
        else
            idL = (frq_cal>=10e3);
            Lprox_tmp = squeeze(Lprox(ik,ig,idL))';
        end

        net_L = nn_simpe((frq_L), Lprox_tmp, Nnlayer-1);
        Lprox(ik,ig,Nfcal+1:Nf) = net_L((frq_nn));
        % check the error
        for ih = Nfcal+1:Nf
            if Lprox(ik,ig,ih)<=0
                Lprox(ik,ig,ih) = Lprox(ik,ig,ih-1);
            else
                Lprox(ik,ig,ih) = max(min(Lprox(ik,ig,ih),Lprox(ik,ig,ih-1)),1/4*Lprox(ik,ig,ih-1));
            end
        end
    end
end
...
for ih = Nfcal+1:Nf
    Lprox(:,:,ih) = Lprox(:,:,ih)+Lprox(:,:,ih)'-diag(diag(Lprox(:,:,ih)));
end


RmeshMF(:,:,Nfcal+1:Nf) = Rmtx(:,:,Nfcal+1:Nf) + Rprox(:,:,Nfcal+1:Nf);
LmeshMF(:,:,Nfcal+1:Nf) = Lmtx(:,:,Nfcal+1:Nf) + Lprox(:,:,Nfcal+1:Nf);
