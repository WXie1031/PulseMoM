function [Gxx,Gzx,Gzz,Gphi] = dgf_main_num_diff_bak_i(...
...
%  Function:       dgf_main_num_diff_bak_i
%  Description:
%
...
for ik = 1:Ns
    ...
    for ig = 1:Nkp-1
        ...
        zm1 = zbdy(id_f);
        zm2 = zbdy(id_f-1);
        TVemm = dgf_kernal_TVmm(zf, zm1,zm2,d_lyr(id_f), GRe_ij(id_f,ind_int), kz_int(id_f,1:Nint));
        TVhmm = dgf_kernal_TVmm(zf, zm1,zm2,d_lyr(id_f), GRh_ij(id_f,ind_int), kz_int(id_f,1:Nint));
        
        TIemm = dgf_kernal_TImm(zf, zm1,zm2,d_lyr(id_f), GRe_ij(id_f,ind_int), Ze(id_f,ind_int), kz_int(id_f,1:Nint));
        TIhmm = dgf_kernal_TImm(zf, zm1,zm2,d_lyr(id_f), GRh_ij(id_f,ind_int), Zh(id_f,ind_int), kz_int(id_f,1:Nint));
        
        Temn_V = dgf_kernal_Tmn(id_s,id_f, d_lyr, GRe_ij(1:Nbdy,ind_int), kz_int(1:Nlyr,1:Nint), 'V');
        Thmn_V = dgf_kernal_Tmn(id_s,id_f, d_lyr, GRh_ij(1:Nbdy,ind_int), kz_int(1:Nlyr,1:Nint), 'V');
        Temn_I = dgf_kernal_Tmn(id_s,id_f, d_lyr, GRe_ij(1:Nbdy,ind_int), kz_int(1:Nlyr,1:Nint), 'I');
        Thmn_I = dgf_kernal_Tmn(id_s,id_f, d_lyr, GRh_ij(1:Nbdy,ind_int), kz_int(1:Nlyr,1:Nint), 'I');
        ...
        Vih_nn = repmat(Zh(id_s,ind_int),Nf,1)/2.*(Ah0+Ah1+Ah2+Ah3+Ah4); 
        Vie_nn = repmat(Ze(id_s,ind_int),Nf,1)/2.*(Ae0+Ae1+Ae2+Ae3+Ae4); 

        sign_A0 = sign(zf-zs(ik));
        sign_A0(sign_A0==0) = 1;
        sign_A0 = repmat(sign_A0,1,Nint);
        Vvh_nn = 1/2*(sign_A0.*Ah0+Ah1-Ah2+Ah3-Ah4);
        Vve_nn = 1/2*(sign_A0.*Ae0+Ae1-Ae2+Ae3-Ae4);

        % Gxx
        Vih_J0_tmp = Vih_nn.*TVhmm.*Thmn_V.*J0_tmp.*repmat(kp_int,Nf,1)*dkp_seg;

        Gxx_num = Gxx_num + sum(Vih_J0_tmp.*repmat(gau_wei,Nf,1) ,2);

        % Gzx
        Vzh_nn = 1/2*(sign_A0.*Ah0-Ah1+Ah2+Ah3-Ah4);
        Vze_nn = 1/2*(sign_A0.*Ae0-Ae1+Ae2+Ae3-Ae4);
        Iih_J1_tmp = Vzh_nn.*TIhmm.*Thmn_I.*J1_tmp.*repmat(kp_int.^2,Nf,1)*dkp_seg;
        kz_m = repmat(kz_int(id_s,1:Nint),Nf,1);
        kp2 = repmat(kp_int.^2,Nf,1);
        km2_over_kzm2 = (kp2 + kz_m.^2) ./ (kz_m.^2);
        Iie_J1_tmp = (Vze_nn.*km2_over_kzm2).*TIemm.*Temn_I.*J1_tmp.*kp2*dkp_seg;

        Gzx_num = Gzx_num + sum( (Iih_J1_tmp-Iie_J1_tmp).*repmat(gau_wei,Nf,1) ,2 );

        % Gzz
        Ivh_J0_tmp = Vvh_nn.*TIhmm.*Thmn_I .* J0_tmp./repmat(kp_int,Nf,1)*dkp_seg;

        Ive_J0_tmp_A = Vve_nn.*TIemm.*Temn_I .* J0_tmp.*repmat(kp_int,Nf,1)*dkp_seg;
        Ive_J0_tmp_B = kz_int(id_f,1:Nint).^2 .* Vve_nn.*TIemm.*Temn_I ...
        ...
        % Gphi
        Vie_J0_tmp = Vie_nn.*TVemm.*Temn_V .* J0_tmp./repmat(kp_int,Nf,1)*dkp_seg;
        Vih_J0_tmp = Vih_nn.*TVhmm.*Thmn_V .* J0_tmp./repmat(kp_int,Nf,1)*dkp_seg;
    ...
    Gzx(ik,indzx) = 1/(2*pi) * 1/(1j*w0*mu0) * -mur_lyr(id_s) * Gzx_num ; 
