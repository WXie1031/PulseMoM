function Txnm = dgf_kernal_Tnm(id_s,id_f, d_lyr, GRx_ji, kz, mode)
%  Function:       dgf_kernal_Tnm
%  Description:
%
...
%             ./ (1+repmat(GRx_ij(ik,1:Nint),Nid_mn,1).*exp(-2*1j*repmat(kz(ik,1:Nint),Nid_mn,1).*d_lyr(ik)));
        Txnm_tmp(ig,1:Nint) = Txnm_tmp(ig,1:Nint) .* (1+s*GRx_ji(ik,1:Nint)) ...
            .*exp(-1j*kz(ik,1:Nint).*d_lyr(ik)) ...
            ./ (1+s*GRx_ji(ik,1:Nint).*exp(-2*1j*kz(ik,1:Nint).*d_lyr(ik)));
    end
    
    % duplicate the calculated value to the matrix
    id_nm = id_f==idf_uni(ig);
    Nid_mn = sum(id_nm);
    Txnm(id_nm,1:Nint) = repmat(Txnm_tmp(ig,1:Nint) ,Nid_mn,1) ;
end
