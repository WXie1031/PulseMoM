function [Ax0,Ax1,Ax2,Ax3,Ax4] = dgf_kernal_A(dz0,dz1,dz2,dz3,dz4, Dx,GRx_ij,GRx_ji, kz)
...
Ax0 = exp(-1j*repmat(kz,Nf,1).*repmat(dz0,1,Nint));
Ax1 = repmat(GRx_ij,Nf,1)./repmat(Dx,Nf,1).*exp(-1j*repmat(kz,Nf,1).*repmat(dz1,1,Nint));
Ax2 = repmat(GRx_ji,Nf,1)./repmat(Dx,Nf,1).*exp(-1j*repmat(kz,Nf,1).*repmat(dz2,1,Nint));
Ax3 = repmat(GRx_ij,Nf,1).*repmat(GRx_ji,Nf,1)./repmat(Dx,Nf,1).*exp(-1j*repmat(kz,Nf,1).*repmat(dz3,1,Nint));
Ax4 = repmat(GRx_ij,Nf,1).*repmat(GRx_ji,Nf,1)./repmat(Dx,Nf,1).*exp(-1j*repmat(kz,Nf,1).*repmat(dz4,1,Nint));
