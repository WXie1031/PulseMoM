function [Ax0,Ax1,Ax2,Ax3,Ax4] = dgf_kernal_Asub(dz0,dz1,dz2,dz3,dz4, Rx_ij,Rx_ji, kz)
...
Ax1 = Rx_ij.*exp(-1j*repmat(kz,Nf,1).*repmat(dz1,1,Nint));
Ax2 = Rx_ji.*exp(-1j*repmat(kz,Nf,1).*repmat(dz2,1,Nint));
Ax3 = Rx_ij.*Rx_ji.*exp(-1j*repmat(kz,Nf,1).*repmat(dz3,1,Nint));
Ax4 = Rx_ij.*Rx_ji.*exp(-1j*repmat(kz,Nf,1).*repmat(dz4,1,Nint));
