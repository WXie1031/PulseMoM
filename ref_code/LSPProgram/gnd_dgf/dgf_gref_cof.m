function out = dgf_gref_cof(kz, ep_lyr, mu_lyr, id_lyr_s, Nbdy,w0)

% [GRe_ij,GRe_ji,De,Dh]

Ze1 = dgf_Ze(kz, ep_lyr(Nbdy), w0);
Ze2 = dgf_Ze(kz, ep_lyr(Nbdy-1), w0);
Re_12 = dgf_Rij(Ze1, Ze2);

Zh1 = dgf_Zh(kz, ep_lyr(1), w0);
Zh2 = dgf_Zh(kz, ep_lyr(2), w0);
Rh_12 = dgf_Rij(Zh1, Zh2(x));

GRe_ij = Re_12;
GRh_ij = Rh_12;

for ig = Nbdy-1:-1:id_lyr_s
    Zei = dgf_Ze(kz, ep_lyr(ig), w0);
    Zej = dgf_Ze(kz, ep_lyr(ig), w0);
    Re_ij = dgf_Rij(Zei, Zej);
    
    Zhi = dgf_Zh(kz, mu_lyr(ig), w0);
    Zhj = dgf_Zh(kz, mu_lyr(ig), w0);
    Rh_ij = dgf_Rij(Zhi, Zhj);
...
for ig = 2:Nbdy
    ...
    GRe_ji = dgf_bdy_gcof_sub(Re_ji(x),GRe_ji(x), kzz(x), d_lyr(ig) );
    GRh_ji = dgf_bdy_gcof_sub(Rh_ji(x),GRh_ji(x), kzz(x), d_lyr(ig) );
end
