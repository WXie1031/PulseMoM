...
if strcmp(opts.method,'FMP') && strcmp(opts.parametertype,'S')
...
%         s_pass=s; %For plotting purposes
        if isfield(opts.plot,'xlim')
            xlimflag=1;
        else
            xlimflag=0;
        end
        if isfield(opts.plot,'ylim')
            ylimflag=1;
        else
            ylimflag=0;
        end
...
if plotte==1
    ...
    figure(7),h0=plot(s_pass/(2*pi*1i),EE0.','b');
    hold on, grid on,
    if xlimflag==1
        xlim([opts.plot.xlim]);
    else
        xlim([s_pass(1)/(2*pi*1i) s_pass(end)/(2*pi*1i)]);
    end
    ...
    figure(8),h2=plot(s_pass/(2*pi*1i),EE0.','b');
    hold on, grid on,
    if xlimflag==1
        xlim([opts.plot.xlim]);
    else
...
for iter_out=1:Niter_out
    ...
    for iter_in=1:1:Niter_in+1
        ...
        if iter_in==1, %######
            ...
            if opts.parametertype=='Y'
                if isempty(wintervals) && all(eig(SER1.D)>=0) && all(eig(SER1.E)>=0)
                    SER0=SER1;
                    break_outer=1;
                    break
                end
            elseif opts.parametertype=='S'
                if isempty(wintervals) && all(svd(SER1.D)<=1)
                    SER0=SER1;
                    break_outer=1;
                    break
                end
            ...
            if opts.parametertype=='Y'
                ...
                if isempty(s2) && all(eig(SER1.D)>0)
                    break
                end
            elseif opts.parametertype=='S'
                ...
                s2=s_viol.'; s2=sort(s2);
                SER1=SER0;
                if isempty(s2) && all(svd(SER1.D)<1)
                    break
                end
            end
            %s2=2*pi*i*1e2;
        ...
        if iter_in==1
            ...
            if outputlevel==1
                if opts.parametertype=='Y'
                    if min(g_pass)<0
                        disp(['    Max. violation, eig(G) : ' num2str(g_pass) ' @ ' num2str((ss)/(2*pi*1i)) ' Hz'])
                    else
                ...
                elseif opts.parametertype=='S'
                    if max(g_pass)>1
                        disp(['    Max. violation, sing(S) : ' num2str(-1+g_pass) ' @ ' num2str((ss)/(2*pi*1i)) ' Hz'])
                    else
        ...
        if plotte==1
            ...
            figure(8),h2=plot(s_pass/(2*pi*1i),EE0.','b-'); hold on
            figure(8),h3=plot(s_pass/(2*pi*1i),EE1.','r--');
            if xlimflag==1
                xlim([opts.plot.xlim]);
            else
                xlim([s_pass(1)/(2*pi*1i) s_pass(end)/(2*pi*1i)]);
...
if plotte==1
    ...
    figure(7),h1=plot(s_pass/(2*pi*1i),EE1.','r--');
    ...
    legend([h0(1) h1(1)],'Original','Perturbed'); hold off
...
ind=find(real(wS1)==0);
wS1=imag(wS1(ind));
singulars=1j*sort(wS1);
A=Acmplx; B=Bcmplx;C=Ccmplx;
E=zeros(Nc,Nc); %Dummy E-matrix
%@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@


if isempty(singulars)
    sing_w=[];
    intervals=[];
    return
end
