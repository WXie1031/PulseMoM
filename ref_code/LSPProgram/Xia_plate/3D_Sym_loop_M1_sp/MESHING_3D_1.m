...
% Oxy(xo,yo),Sxy(x1,x2,y1,y2)
% Oxy_X(xo1,xo2,yo),Sxy_X(x1,x2,y1,y2)
% Oxy_Y(xo,yo1,yo2),Sxy_Y(x1,x2,y1,y2)
Nx=length(u)-1;
Ny=length(v)-1;
NXx=length(ux)-1;
NXy=length(vx)-1;
...
Sxy2=[Sxy4(:,1:2) Sxy1(:,3:4)];
Sxy3=[Sxy1(:,1:2) Sxy4(:,3:4)];
...
for i=1:NXyc  
    ...
    Sxy1_X(idx,2)=ux(2:NXxc+1);
    Sxy1_X(idx,3)=vx(i);
    Sxy1_X(idx,4)=vx(i+1); 
    Sxy4_X(idx,1)=fliplr(ux(NXxc+2:NXx));
    Sxy4_X(idx,2)=fliplr(ux(NXxc+3:NXx+1));
    Sxy4_X(idx,3)=vx(NXy+1-i);
    Sxy4_X(idx,4)=vx(NXy+2-i);
    Sxy5_X(i,1)=ux(NXxc+1);
    Sxy5_X(i,2)=ux(NXxc+2);
    Sxy5_X(i,3)=vx(i);
    Sxy5_X(i,4)=vx(i+1); 
    Sxy6_X(i,3)=vx(NXy+1-i);
    Sxy6_X(i,4)=vx(NXy+2-i);
end
Sxy2_X=[Sxy4_X(:,1:2) Sxy1_X(:,3:4)];
Sxy3_X=[Sxy1_X(:,1:2) Sxy4_X(:,3:4)];
...
for i=1:NYyc
    ...
    Sxy1_Y(idx,2)=uy(2:NYxc+1);
    Sxy1_Y(idx,3)=vy(i);
    Sxy1_Y(idx,4)=vy(i+1); 
    Sxy4_Y(idx,1)=fliplr(uy(NYxc+1:NYx));
    Sxy4_Y(idx,2)=fliplr(uy(NYxc+2:NYx+1));
    Sxy4_Y(idx,3)=vy(NYy+1-i);
    Sxy4_Y(idx,4)=vy(NYy+2-i);   
end
Sxy2_Y=[Sxy4_Y(:,1:2) Sxy1_Y(:,3:4)];
Sxy3_Y=[Sxy1_Y(:,1:2) Sxy4_Y(:,3:4)];
Sxy5_Y=[Sxy1_Y(1:NYxc,1:2) Sxy1_Y(end-NYxc+1:end,4) Sxy3_Y(end-NYxc+1:end,3)];
Sxy6_Y=[Sxy4_Y(1:NYxc,1:2) Sxy5_Y(:,3:4)];
...
for i=1:NZyc
    ...
    Sxy1_Z(idx,2)=uz(2:NZxc+1);
    Sxy1_Z(idx,3)=vz(i);
    Sxy1_Z(idx,4)=vz(i+1); 
    Sxy4_Z(idx,1)=fliplr(uz(NZxc+1:NZx));
    Sxy4_Z(idx,2)=fliplr(uz(NZxc+2:NZx+1));
    Sxy4_Z(idx,3)=vz(NZy+1-i);
    Sxy4_Z(idx,4)=vz(NZy+2-i);
end
Sxy2_Z=[Sxy4_Z(:,1:2) Sxy1_Z(:,3:4)];
Sxy3_Z=[Sxy1_Z(:,1:2) Sxy4_Z(:,3:4)];

Oxy_Z(:,1)=0.5*(Sxy1_Z(:,1)+Sxy1_Z(:,2));
Oxy_Z(:,2)=0.5*(Sxy1_Z(:,3)+Sxy1_Z(:,4));
