% calculate the affect on the objective line from Mz in the edge blocks 
%
% updated on 31/03/2011
%
function [Pmxz0 Pmyz0 Pmzz0]=BFCAL_M0(Oxyz,Sxyz_Z0)

[NZ0 ~]=size(Sxyz_Z0);
NZ0=NZ0/4;

% direct integration for solving Pmxz Pmyz Pmzz
[I16]=FM_3D_16_xz(Oxyz,Sxyz_Z0);
Pmxz0=I16;

Pmxz0=Pmxz0(:,1:NZ0)+Pmxz0(:,NZ0+1:2*NZ0)+Pmxz0(:,2*NZ0+1:3*NZ0)+Pmxz0(:,3*NZ0+1:4*NZ0);

[I15]=FM_3D_15_yz(Oxyz,Sxyz_Z0);
Pmyz0=I15;

Pmyz0=Pmyz0(:,1:NZ0)+Pmyz0(:,NZ0+1:2*NZ0)+Pmyz0(:,2*NZ0+1:3*NZ0)+Pmyz0(:,3*NZ0+1:4*NZ0);

[I13]=FM_3D_13_0(Oxyz,Sxyz_Z0);
Pmzz0=I13;

Pmzz0=Pmzz0(:,1:NZ0)+Pmzz0(:,NZ0+1:2*NZ0)+Pmzz0(:,2*NZ0+1:3*NZ0)+Pmzz0(:,3*NZ0+1:4*NZ0);

clear I16 I15 I13;

end