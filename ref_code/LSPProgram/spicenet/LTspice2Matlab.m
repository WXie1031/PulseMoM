...
%
    ...
    while 1,   
        ...
        if ~isempty(strfind( the_line, 'Binary:' )),  file_format = 'binary';  break;  end
        if ~isempty(strfind( the_line, 'Values:' )),  file_format = 'ascii';   break;  end
        ...
        if variable_flag==0,  %Non-variable header section
            ...
            vn_keep_index =  var_name~=' ' & var_name~='.' & var_name~=char(9) & var_name~=char(10) & var_name~=char(13) ;
    ...
    if strcmpi( simulation_type, '.tran' ) && (min(diff(raw_data.time_vect)) < 0.0),   %Check to see if the time vector is monotonically increasing.
        ...
        if downsamp_N~=1,  %If we have already downsampled then we can't uncompress.
            ...
            new_index =  ~nan_vect ;  %Index into time_vect_new indicating the new points only.
            old_index =  nan_vect ;
