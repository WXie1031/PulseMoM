function [Uout,Er_T,Ez_T] = sr_induced_v_num(pt_hit,h_ch,Ns_ch, pt_start,pt_end, ...
    i_sr,t_sr, flag_type)
%  Function:       sr_induced_v_num
...
%                  Static field is removed for field calculation.
%
%  Calls:          
%
...
%z_ch = ((Ns_ch:-1:1)-0.5)'*dz_ch; % mid point of the channel segment
...
% is add abs() or not is still a problem
for ik = 1:Nc
    Uout(ik,1:Nt)= Er_T(ik,1:Nt).*Rx(ik)/Rxy(ik)*(dx(ik)) ...
        + Er_T(ik,1:Nt)*Ry(ik)/Rxy(ik)*(dy(ik)) ...
        + Ez_T(ik,1:Nt)*(dz(ik));
end
