function optimize_double_exp_source(sr_type, amp, trise, tfall, td, ...
...
[sr_amp, sr_alpha, sr_beta] = sr_double_exp_para(amp, trise, tfall);
% sr_alpha - us
% sr_beta  - us

Tmax = max(tfall*1.5,50);     % us
dt = min(1e-1,trise/200);  % us
Nt = ceil((Tmax)/dt);
tus = (0:Nt-1)*dt; % us
...
if td>0
    ...
    ist(Ntd+1:end) = sr_amp  * (exp(-sr_alpha*(tus))-exp(-sr_beta*(tus)));
    tus = (0:Ntd+Nt-1)*dt;
else
    ist = sr_amp  * (exp(-sr_alpha*tus)-exp(-sr_beta*tus));
end
...
sr_alpha = sr_alpha*1e6;% transform into [s] 
sr_beta = sr_beta*1e6;% transform into [s] 
sr_tau1 = 1/sr_alpha;
sr_tau2 = 1/sr_beta;
...
spice_double_exp_source(sr_type, sr_amp, sr_tau1, sr_tau2, td, pathstr, fname);
