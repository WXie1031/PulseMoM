function [Rsg,Lsg, Zg,Yg] = grid_tline_v(re, sig, mur, sig_soil,epr_soil, frq)
%  Function:       grid_tline_v
...
for ik = 1:Nc
    ...
    Zin = zin_cir_ac(re(ik), 0, sig(ik), mur(ik), 1, frq);
    
    for ig = 1:Nf
        K = fminsearch(@(K) grid_tline_v_sub(Zin(ig), re(ik), kg(ig), rg(ig), wm(ig), K),...
            [1e-4+1e-8i*wm(ig)], ...
            optimset('TolFun',1e-8,'MaxIter',10000,'MaxFunEvals',10000));
        
        Zg(ik,ig) = Zin(ig) + 1j*wm(ig)*mu0/(2*pi).* log(1.12./( sqrt(K.^2+rg(ik).^2).*re(ik)) );
        Yg(ik,ig) = 2*pi*kg(ig) / log((sqrt(K.^2+rg(ig).^2).*re(ik))/3.56);
    end
