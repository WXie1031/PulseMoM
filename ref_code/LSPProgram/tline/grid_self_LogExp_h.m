function [Rgrid, Lgrid] = grid_self_LogExp_h(Rgrid, Lgrid, pt_start_grid, pt_end_grid, ...
    dv_grid, re_grid, len_grid, sig_soil, epr_soil, frq)
%  Function:       grid_self_LogExp_h
%  Description:    Calculate self R, L of horizontal grid conductors using
%                  LogExp model in paper "External Impedance and Admittance
...
rg_re = re_grid(id_grd_h)*rg;

Zg = 1j*mu0/(2*pi)*wm_tmp.*( log((1+rg_re)./rg_re) ...
    + 2*exp(-2*depth*abs(rg))./(4+rg_re.^2) );
