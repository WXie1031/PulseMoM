function [Rgrid, Lgrid] = grid_self_Sunde_log_h(Rgrid, Lgrid, pt_start_grid, pt_end_grid, ...
    dv_grid, re_grid, len_grid, sig_soil, epr_soil, frq)
%  Function:       grid_self_Sunde_log_h
...
%Zg = 1j*mu0/(2*pi)*wm_tmp .* log( (1+rg.*re_grid)./(rg.*re_grid) );
Zg = 1j*mu0/(2*pi)*wm_tmp .* log( 1 + 1./(rg.*re_grid) );
