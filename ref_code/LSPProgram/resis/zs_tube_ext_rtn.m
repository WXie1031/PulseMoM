function [R, Lin, Zin] = zs_tube_ext_rtn(r_out,r_in,sig,mur, len, frq)
%  Function:       zin_cir_ext_rtn
%  Description:    Calculate R and L of circular conductors using bessel
%                  function. Corresponding to Zbb in "The Electromagnetic
...
for ik = 1 : Na
    if r_in(ik) == 0
        ...
        % self impedance
        Zin(ik,1:Nf) = 1j*w*mu(ik)./(2*pi*Ro) .* (I0o./I1o) .*len(ik);
        ...
        % self impedance
        Zin(ik,1:Nf) = 1j*w*mu(ik)./(2*pi*Ri) .* ...
            ( (I0i.*K1o+K0i.*I1o)./(I1o.*K1i-K1o.*I1i) ) .*len(ik);
        ...
        if ~isempty(ind)           
            ...
            %Zin(ik,end) = 1j*w(end)*mu/(2*pi*Ri(end))*coth(Ro(end)-Ri(end));
