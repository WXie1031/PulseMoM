function Rskin = resis_agi_ac( wid, hig, R_pul, sig_pul, mur, len, frq )
...
%wcmap = 1./(2*(wid-hig)./hig + (1-2*log(2)/pi));
wagi =0.4*(2*wid-hig);
%mur=1;
s_dep = 1./sqrt(pi*frq.*sig_pul.*mu0.*mur);

hw_ratio = max(hig./wagi, wagi./hig);
...
xw = sqrt(2/pi).*sqrt(wagi.*hig)./s_dep;
%xw = 3.5.*sqrt(wid.*hig)./s_dep;

k01 = 0.58093*xw./(1+0.041*hw_ratio.^1.19) + (1.1147+1.2868*xw)./(1.2296+1.287*xw.^3)...
