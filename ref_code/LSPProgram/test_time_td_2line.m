...
[~, Lpeec, Ppeec] = para_main_fila(pt_sta_L, pt_end_L, dv_L, d1_L, l_L, ...
...
Lpeec = sparse(Lpeec+diag(Ls*ones(Nseg,1)-diag(Lpeec)));
Rpeec = sparse(diag(Rs*ones(Nseg,1)));
Rfit = ones(Nseg,1)*Rn;
Lfit = ones(Nseg,1)*Ln;
...
plot(tus(1:Nt-1), Ibt(50,1:Nt-1));
plot(tus(1:Nt-1), Ibt(150,1:Nt-1));
plot(tus(1:Nt-1), Ibt(200,1:Nt-1));
