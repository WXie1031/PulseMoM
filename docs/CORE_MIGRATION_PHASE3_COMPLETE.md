...
已更新以下文件的引用：
1. **`mom_solver_unified.c`**: 
   - `core/core_assembler.h` → `operators/assembler/core_assembler.h`
   - `core/electromagnetic_kernels.h` → `operators/kernels/electromagnetic_kernels.h`
2. **`mom_solver_min.c`**: `core/core_assembler.h` → `operators/assembler/core_assembler.h`
3. **`peec_solver_unified.c`**: 
   - `core/core_kernels.h` → `operators/kernels/core_kernels.h`
   - `core/core_assembler.h` → `operators/assembler/core_assembler.h`
4. **`peec_directionality_kernels.c`**: `core/core_kernels.h` → `operators/kernels/core_kernels.h`
5. **`hybrid_solver.c`**: `core/core_kernels.h` → `operators/kernels/core_kernels.h`
6. **`mtl_hybrid_coupling.c`**: `core/core_kernels.h` → `operators/kernels/core_kernels.h`
7. **`mtl_coupling/mtl_hybrid_coupling.c`**: `core/core_kernels.h` → `operators/kernels/core_kernels.h`
